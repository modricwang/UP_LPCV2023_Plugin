...
class Learnable_Relu(nn.Module):
    def __init__(self, slope_init=0., inplace=False):
    ...
    def forward(self, x):
        if self.tocaffe:
            x = F.leaky_relu(x, negative_slope=self.slope_param.data.item())
        else:
...
class Learnable_Relu6(nn.Module):
    ...
    def forward(self, x):
        slope = (
                        self.slope_param - self.slope_param * self.slope_lr_scale).detach() + self.slope_param * self.slope_lr_scale
