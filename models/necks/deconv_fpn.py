...
def build_conv_norm_dbb(in_channels,
    ...
    conv = DiverseBranchBlock(
        ...
        groups=groups, normalize=normalize)
    return conv
    # # for compability
...
@MODULE_ZOO_REGISTRY.register('Deconv_FPN')
class Deconv_FPN(nn.Module):
    """
    ...
    def __init__(self,
        ...
        super(Deconv_FPN, self).__init__()
        ...
        if upsample == 'deconv':
            if self.split_deconv:
                self.deconv = []
                for lvl_idx in range(max(num_level,len(inplanes))):
                    self.deconv.append(nn.ConvTranspose2d(self.outplanes,
                                                          self.outplanes,
                ...
                self.deconv = nn.ConvTranspose2d(self.outplanes,
                                                 ...
                                                 stride=2,
                                                 padding=1)
        build_func = build_conv_norm if not use_dbb else build_conv_norm_dbb
        for lvl_idx in range(max(num_level,len(inplanes))):
            if lvl_idx < len(inplanes):
                planes = inplanes[lvl_idx]
                self.add_module(
                    self.get_lateral_name(lvl_idx),
                    DiverseBranchBlock(planes, outplanes, 1,
                                       internal_channels_1x1_3x3=outplanes * dbb_ratio,
                                       normalize=normalize) if use_dbb else build_func(
                        planes, outplanes, 1, normalize=normalize))
                self.add_module(
                    self.get_pconv_name(lvl_idx),
                    DiverseBranchBlock(outplanes, outplanes, kernel_size=3, stride=1, padding=1,
                                       internal_channels_1x1_3x3=outplanes * dbb_ratio,
                                       normalize=normalize) if use_dbb else
                    build_func(outplanes, outplanes, kernel_size=3,
                ...
                if self.downsample == 'pool':
                    ...
                    self.add_module(
                        self.get_downsample_name(lvl_idx),
                        DiverseBranchBlock(outplanes, outplanes, kernel_size=3, stride=2, padding=1,
                                           internal_channels_1x1_3x3=outplanes * dbb_ratio,
                                           normalize=normalize) if use_dbb
                        else
            ...
            if lvl_idx < len(self.inplanes) - 1:
                if self.upsample == 'deconv':
                    if self.split_deconv:
                        ...
                        try:
                            laterals[lvl_idx] += self.deconv[lvl_idx](laterals[lvl_idx + 1])
                        except RuntimeError as e:
                            print(lvl_idx, laterals[lvl_idx].shape, self.deconv[lvl_idx](laterals[lvl_idx + 1]).shape)
                            import sys
                            sys.stdout.flush()
                            raise e
                    else:
